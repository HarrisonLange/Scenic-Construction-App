...
children.push(
  ...
  bullet([{ text: "Progress follows the student ID. ", bold: true }, { text: "Lab completion and scores sync to the school-hosted progress store, while projects and the optional certificate name remain in that browser. Students can also choose Local only when they should not use an ID (details under Logistics & Privacy)." }]),
  rich([
  ...
  h3("Devices & setup"),
  bullet("Any modern browser on Chromebooks, PCs, or Macs; Chrome or Edge recommended. No installation or password-based account is required."),
  ...
  bullet("Lab completion and best scores sync under the typed student ID, so progress follows the student to another device. A new ID is added to tracking automatically."),
  bullet("Projects, imported media, certificate names, and other detailed lab work stay in that browser. Local only also keeps progress in the browser; it does not transfer and may be lost if site data is cleared or the device is reset."),
  ...
  bullet("On shared computers, students should use Switch profile before handing off the device. Have students confirm the certificate name is theirs. The hub’s Reset button permanently clears the current ID’s tracked progress on every device."),
  h3("Privacy"),
  bullet("The school-hosted progress store records the typed student ID, lab completion, best score, and activity dates. It does not collect analytics, projects, imported media, or certificate names."),
  bullet("Local only does not send an ID or progress to the server. Use it when server tracking is not appropriate, with the understanding that its browser data is less durable."),
  ...
  bullet([{ text: "“My progress disappeared” ", bold: true }, { text: "— confirm the exact same student ID was entered. In Local only, a different browser, cleared site data, or a device reset removes progress. Downloaded artifacts remain the durable evidence." }]),
