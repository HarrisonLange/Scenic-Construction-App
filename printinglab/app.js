...
function createBambuStudioCompatibleGcode(gcode, machine, material, layers, bounds) {
...
async function sliceModel() {
  ...
  try {
    ...
    const result = Object.freeze({
      ...
      quality,
      supportsEnabled: elements.supportsToggle.checked,
    });
    updateState({ result });
    ...
    showStatus("success", "✓", "Bambu print file ready", `Checked for ${machine.name} and AMS use. Open the download in Bambu Studio to map ${material.code} to an AMS slot.`, Number.NaN);
  } finally {
    setBusy(false);
  }
}
